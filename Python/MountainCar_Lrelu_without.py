...
class DQNAgent:
    ...
    def replay(self):
        ...
        for i in range(self.batch_size):
            ...
            clipping.append([-1, max(target[i])])


    
        fit_minibatch(self.model, state.tolist(), target, self.batch_size,self.learning_rate, clipping )
...
if __name__  == "__main__":
    ...
    # agent.load('D:/DoAn/Code_PY/golden_weight/mountain_car_220213')
    ...
    for e in range(EPISODES):
        ...
        while not done:
            ...
            reward = 100*((np.sin(3 * next_state[0]) * 0.0025 + 0.5 * next_state[1] * next_state[1]) - (np.sin(3 * state[0][0]) * 0.0025 + 0.5 * state[0][1] * state[0][1])) 
            if next_state[0] >= 0.5:
                reward += 1  
            # if done:
            #     reward = reward + 10000
            ...
            if done:
                ...
                print("episode: {}/{}, score: {}, e: {:.2}, time: {}"
                      .format(e, EPISODES, scores, agent.epsilon, time))
        ...
        if e % 200 == 0:
            ...
            agent.save("D:/DoAn/Code_PY/weight_train/mountainCar_withLrelu_220313")
